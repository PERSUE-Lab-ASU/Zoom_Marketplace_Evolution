...
def filter_apps_by_category(data, category):
    """Filter apps by a specific category."""
    return [app for app in data if category in app.get('categories', [])]


def prepare_combined_permissions_upset_data(apps):
    """Prepare UpSet data for view and manage permissions."""
...
# Filter apps in the Learning & Development category
learning_apps = filter_apps_by_category(data, "Learning & Development")

# Prepare UpSet data for combined view and manage permissions
upset_data_combined_permissions = prepare_combined_permissions_upset_data(learning_apps)
...
if not upset_data_combined_permissions.empty:
    ...
    upset_plot = UpSet(upset_data_combined_permissions,
                       ...
                       min_subset_size=1)
    upset_plot.plot()
    plt.title('View and Manage Permissions for Learning & Development Apps (Dec 15, 2024)')
    plt.show()
else:
    print("No data available for view or manage permissions in Learning & Development apps.")
