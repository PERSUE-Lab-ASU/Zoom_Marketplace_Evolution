...
def prepare_permissions_upset_data(data1, data2):
    app_permissions1 = {app['appName']: set(app.get('managePermissions', [])) for app in data1}
    app_permissions2 = {app['appName']: set(app.get('managePermissions', [])) for app in data2}
...
(upset_data_new, upset_data_modified_added, upset_data_modified_removed, upset_data_deleted,
 ...
 single_new, single_modified_added, single_modified_removed) = prepare_permissions_upset_data(
    data_may_12_2024, data_dec_15_2024)
...
if not upset_data_new.empty:
    ...
    upset_new = UpSet(upset_data_new,
                      ...
                      min_subset_size=1)
    upset_new.plot()
    plt.title(f'Manage Permission Combinations in New Apps ({date1} to {date2})\nTotal New Apps with Permissions: {new_count}')
    plt.show()
...
if not upset_data_modified_added.empty:
    ...
    plt.title(f'Manage Permission Combinations Added to Existing Apps ({date1} to {date2})\nTotal Apps: {modified_added_count}')
...
if not upset_data_modified_removed.empty:
    ...
    plt.title(f'Manage Permission Combinations Removed from Existing Apps ({date1} to {date2})\nTotal Apps: {modified_removed_count}')
    plt.show()
...
if not upset_data_deleted.empty:
    ...
    upset_deleted = UpSet(upset_data_deleted,
                         ...
                         min_subset_size=1)
    upset_deleted.plot()
    plt.title(f'Manage Permission Combinations in Deleted Apps ({date1} to {date2})\nTotal Apps: {deleted_count}')
    plt.subplots_adjust(right=0.70)
    plt.show()

# Print summary
print(f"\nSummary of Manage Permission Changes ({date1} to {date2}):")
...
print("\nSingle Manage Permission Additions in New Apps:")
for permission, count in single_new.most_common():
    print(f"{permission}: {count} apps")

print("\nSingle Manage Permission Additions in Modified Apps:")
for permission, count in single_modified_added.most_common():
    print(f"{permission}: {count} apps")

print("\nSingle Manage Permission Removals in Modified Apps:")
