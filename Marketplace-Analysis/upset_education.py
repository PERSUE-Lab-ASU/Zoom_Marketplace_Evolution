...
# Filter apps in the Education category
education_apps = filter_apps_by_category(data, "Education")

# Prepare UpSet data for combined view and manage permissions
upset_data_combined_permissions = prepare_combined_permissions_upset_data(education_apps)
...
if not upset_data_combined_permissions.empty:
    ...
    upset_plot = UpSet(upset_data_combined_permissions,
                       ...
                       min_subset_size=1)
    upset_plot.plot()
    plt.title('View and Manage Permissions for Education Apps (Dec 15, 2024)')
    plt.show()
else:
    print("No data available for view or manage permissions in Education apps.")
